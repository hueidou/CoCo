...
If you believe you've found a security issue in CoCo, please report it privately.

## Reporting

Report vulnerabilities of the CoCo repository:

If you discover a security issue in CoCo, please report it to us through the [Alibaba Security Response Center (ASRC)](https://security.alibaba.com/).
...
7. **Environment** (Python version, OS, how CoCo is run)
...
- Tested version details (CoCo version and/or commit SHA).
- Reproducible PoC against latest `main` or latest released version.
- Demonstrated impact tied to CoCo's documented trust boundaries (see below).
- For exposed-secret reports: proof the credential is CoCo-owned or grants access to CoCo-operated infrastructure/services.
...
- Reports that assume per-user multi-tenant authorization on a shared CoCo instance/config.
...
## Alignment with `coco init`

The security model and recommended baseline in this document are **aligned with** what users see and accept during **`coco init`**. The init flow shows a security notice that covers: single-operator boundary; shared delegated authority when multiple people message the same instance; restricting channels and users (allowlists); using separate config/credentials and OS users or hosts per trust boundary; least privilege and sandboxing; keeping secrets out of the working directory and skill-accessible paths; and reviewing config and skills regularly. When updating either this document or the `SECURITY_WARNING` in `src/coco/cli/init_cmd.py`, keep the **concepts and recommendations** consistent so operators get the same message in both places.

## Security & Trust

Security handling is owned by the CoCo maintainers. For sensitive reports, use the private advisory or a private channel as above.

## Bug Bounties

CoCo is a community open-source project. There is no bug bounty program and no budget for paid reports. Please still disclose responsibly so we can fix issues quickly. The best way to help the project right now is by sending PRs or clear, reproducible reports.

## Operator Trust Model

CoCo does **not** model one instance as a multi-tenant, adversarial user boundary.

- Authenticated callers to the same CoCo instance (same config, same channel workspace) are treated as **trusted operators** for that instance.
...
- **Recommended mode**: one user per machine/host (or per OS user), one CoCo config for that user, and one or more agents/skills inside that instance.
- If multiple users need CoCo, use one host/OS user (or VPS) per user, or strict isolation; sharing one instance by mutually untrusted users is not the recommended default.
- Skills run with the same privileges as the CoCo process; only install and enable skills you trust.


## Trusted Skills Concept

Skills/extensions are part of CoCo's **trusted computing base** for an instance.
...
- Public internet exposure of CoCo when the docs recommend against it.
- Using CoCo in ways that the docs recommend not to.
- Deployments where mutually untrusted/adversarial operators share one CoCo instance and config (e.g. reports expecting per-operator isolation for sessions, history, or similar).
...
- Exposed secrets that are third-party or user-controlled credentials (not CoCo-owned and not granting access to CoCo-operated infrastructure) without demonstrated CoCo impact.
- Scanner-only claims against stale or nonexistent paths, or without a working repro.

## Deployment Assumptions

CoCo security guidance assumes:

- The host where CoCo runs is within a trusted OS/admin boundary.
- Anyone who can modify the CoCo working directory and config (including `config.json`) is effectively a trusted operator.
...
CoCo's security model is **“personal assistant”** (one trusted operator, potentially many agents/skills), not “shared multi-tenant bus.”

- If multiple people can message the same tool-enabled CoCo instance (e.g. a shared channel workspace), they can all steer that agent within its granted permissions.
...
The CoCo working directory is treated as **trusted local operator state**.
...
- Keep secrets out of the working directory and skill-accessible paths; this is part of the baseline recommended in `coco init`.

## Skills Trust Boundary

Skills are loaded and run **in-process** (or under the same trust boundary) as the CoCo runtime and are treated as trusted code.

- Skills can execute with the same OS privileges as the CoCo process.
...
For more operational and hardening guidance, see the [documentation](https://coco.agentscope.io/) and any security-related docs linked from the repo.

## Runtime Requirements

- **Python**: CoCo requires a supported Python version (see [README](README.md)). Use a version with current security updates.
