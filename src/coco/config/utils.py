...
def _normalize_working_dir_bound_paths(data: object) -> object:
    """Normalize legacy ~/.coco-bound paths to current WORKING_DIR.

    This keeps COCO_WORKING_DIR effective even if user config files contain
    older hard-coded paths like "~/.coco/media" or
    "/Users/x/.coco/workspaces/...".
    Only rewrites known working-dir-bound keys.
    """
    legacy_root_tilde = "~/.coco"
...
def get_available_channels() -> Tuple[str, ...]:
    ...
    coco.channels), filtered by COCO_ENABLED_CHANNELS or
    COCO_DISABLED_CHANNELS when set.

    * COCO_ENABLED_CHANNELS — whitelist (only these channels are active).
    * COCO_DISABLED_CHANNELS — blacklist (all channels *except* these).
    * If both are set, COCO_ENABLED_CHANNELS takes precedence.
...
def is_running_in_container() -> bool:
    ...
    Prefer env COCO_RUNNING_IN_CONTAINER (1/true/yes) at call time so
...
def is_coco_running() -> bool:
    """Check if CoCo is currently running by checking API availability.

    Returns:
        True if CoCo is running, False otherwise
    """
    try:
