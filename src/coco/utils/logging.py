# -*- coding: utf-8 -*-
"""Logging setup for CoCo: console output and optional file handler."""
import io
import logging
...
_COCO_LOG_MAX_BYTES = 5 * 1024 * 1024  # 5 MiB
_COCO_LOG_BACKUP_COUNT = 3
...
def setup_logger(level: int | str = logging.INFO):
    """Configure logging to only output from this package (coco), not deps."""
    ...
    # Only attach handler to our namespace so only coco.* logs are printed.
...
def add_coco_file_handler(log_path: Path) -> None:
    """Add a file handler to the coco logger for daemon logs.
    ...
    Args:
        log_path: Path to the log file (e.g. WORKING_DIR / "coco.log").
    """
    ...
    if is_windows_or_linux:
        ...
        file_handler = logging.handlers.RotatingFileHandler(
            log_path,
            encoding="utf-8",
            maxBytes=_COCO_LOG_MAX_BYTES,
            backupCount=_COCO_LOG_BACKUP_COUNT,
        )
