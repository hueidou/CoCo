...
TELEMETRY_ENDPOINT = "https://coco-telemetry-xissagieap.cn-hangzhou.fcapp.run"
...
def _detect_install_method() -> str:
    """Detect how CoCo was installed based on environment signals."""
    import os

    if os.environ.get("COCO_RUNNING_IN_CONTAINER", "").lower() in (
        "1",
        "true",
        "yes",
    ):
        return "docker"
    if os.environ.get("COCO_DESKTOP_APP", "").lower() in (
        "1",
        "true",
        "yes",
    ):
...
def get_system_info() -> dict[str, Any]:
    ...
    - coco_version: CoCo version string
    - install_method: How CoCo was installed (docker/desktop/pip)
    - os: Operating system (Windows/Darwin/Linux)
    - os_version: OS version string
    - python_version: Python version running coco (major.minor)
    - architecture: CPU architecture (x86_64/arm64/etc)
    - has_gpu: GPU availability detection
    """
    from ..__version__ import __version__ as coco_ver

    info = {
        "install_id": str(uuid.uuid4()),
        "coco_version": _safe_get(lambda: coco_ver, "unknown"),
...
def _get_current_version() -> str:
    """Get the current CoCo version string."""
    try:
        from ..__version__ import __version__ as coco_ver
...
def has_telemetry_been_collected(working_dir: Path) -> bool:
    ...
    Re-triggers collection when CoCo is upgraded (or downgraded) to a version
    that hasn't been collected before.

    Args:
        working_dir: Path to CoCo working directory
    ...
    try:
        ...
        # v1.1 compat: single coco_version field
        return marker_data.get("coco_version", "") == current
    except Exception:
        return False
        ...
        if marker_file.exists():
            try:
                ...
                if not collected_versions:
                    old_ver = old_data.get("coco_version", "")
                    if old_ver:
