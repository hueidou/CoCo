...
def _do_migrate_legacy_workspace() -> bool:
    ...
    default_agent_config = AgentProfileConfig(
        id="default",
        name="Default Agent",
        description="Default CoCo agent (migrated from legacy config)",
...
def _migrate_workspace_items_from_source(
    ...
    Args:
        source_dir: Source directory (e.g., ~/.coco or WORKING_DIR)
...
def ensure_qa_agent_exists() -> None:
    ...
    ``coco_source_index``), and built-in tools are restricted (see
    ``build_qa_agent_tools_config``).
...
def _do_ensure_qa_agent() -> None:
    ...
    agent_config = AgentProfileConfig(
        ...
        description=(
            "Builtin Q&A helper for CoCo setup, local config under "
            "COCO_WORKING_DIR, and documentation. Prefer reading files "
