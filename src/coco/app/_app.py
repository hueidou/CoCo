...
from ..utils.logging import setup_logger, add_coco_file_handler
...
async def lifespan(
    ...
    add_coco_file_handler(WORKING_DIR / "coco.log")
...
# Console static dir: env, or coco package data (console), or cwd.
_CONSOLE_STATIC_ENV = "COCO_CONSOLE_STATIC_DIR"


def _resolve_console_static_dir() -> str:
    if os.environ.get(_CONSOLE_STATIC_ENV):
        return os.environ[_CONSOLE_STATIC_ENV]
    # Shipped dist lives in coco package as static data
...
def read_root():
    ...
    return {
        "message": (
            "CoCo Web Console is not available. "
            "If you installed CoCo from source code, please run "
            "`npm ci && npm run build` in CoCo's `console/` "
            "directory, and restart CoCo to enable the "
            "web console."
        ),
    }
...
@app.get("/api/version")
def get_version():
    """Return the current CoCo version."""
...
if os.path.isdir(_CONSOLE_STATIC_DIR):
    ...
    @app.get("/coco-symbol.svg")
    def _console_icon():
        f = _console_path / "coco-symbol.svg"
        if f.is_file():
            return FileResponse(f, media_type="image/svg+xml")
        raise HTTPException(status_code=404, detail="Not Found")

    @app.get("/coco-dark.png")
    def _console_dark_icon():
        f = _console_path / "coco-dark.png"
        if f.is_file():
            return FileResponse(f, media_type="image/png")
        raise HTTPException(status_code=404, detail="Not Found")
