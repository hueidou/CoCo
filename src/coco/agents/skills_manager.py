...
def _extract_requirements(post: dict[str, Any]) -> SkillRequirements:
    ...
    elif "coco" in metadata and isinstance(metadata["coco"], dict):
        requires = metadata["coco"].get("requires", {})
    else:
...
def _skill_config_env_var_name(skill_name: str) -> str:
    ...
    return f"COCO_SKILL_CONFIG_{''.join(normalized).strip('_') or 'DEFAULT'}"
...
def apply_skill_config_env_overrides(
    ...
    ``COCO_SKILL_CONFIG_<SKILL_NAME>`` (JSON string).
    """
...
def _extract_emoji_from_metadata(metadata: Any) -> str:
    """Extract emoji from metadata.coco.emoji."""
    if not isinstance(metadata, dict):
        return ""
    coco = metadata.get("coco", {})
    if isinstance(coco, dict):
        return str(coco.get("emoji", "") or "")
    return ""
...
def _extract_zip_skills(data: bytes) -> tuple[Path, list[tuple[Path, str]]]:
    ...
    tmp_dir = Path(tempfile.mkdtemp(prefix="coco_skill_upload_"))
...
def _staged_skill_dir(skill_name: str) -> Iterator[Path]:
    ...
    temp_root = Path(
        tempfile.mkdtemp(prefix=f"coco_skill_stage_{skill_name}_"),
    )
