# -*- coding: utf-8 -*-
"""Token counting utilities for CoCo using HuggingFace tokenizers.
...
class CocoTokenCounter(HuggingFaceTokenCounter):
    """Token counter for CoCo with configurable tokenizer support.
...
class CocoEstimateTokenCounter(HuggingFaceTokenCounter):
...
_token_counter_cache: dict[
    tuple,
    CocoTokenCounter | CocoEstimateTokenCounter,
] = {}


def get_coco_token_counter(
    ...
    Args:
        ...
        use_estimate: If True (default), returns a CocoEstimateTokenCounter
            that uses only character-based estimation without loading a
            tokenizer. If False, returns a full CocoTokenCounter backed
    ...
    if use_estimate:
        return CocoEstimateTokenCounter(
        ...
        if config_key not in _token_counter_cache:
            _token_counter_cache[config_key] = CocoTokenCounter(
