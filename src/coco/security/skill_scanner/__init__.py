# -*- coding: utf-8 -*-
"""
Skill security scanner for CoCo.
...
def _get_scan_mode(cfg: Any = None) -> str:
    ...
    Priority: env ``COCO_SKILL_SCAN_MODE`` > config > default ``warn``.
    """
    env = os.environ.get("COCO_SKILL_SCAN_MODE")
    if env is not None:
        val = env.lower().strip()
...
def _get_blocked_history_path() -> Path:
    try:
        ...
        return Path.home() / ".coco" / _BLOCKED_HISTORY_FILE
