...
def _check_coco_not_running():
    """Check if CoCo is not running, exit if it is."""
    from ..config.utils import is_coco_running

    if is_coco_running():
        click.echo(
            "❌ CoCo is currently running. Please stop it first:",
            err=True,
        )
        click.echo("   coco shutdown", err=True)
        click.echo(
            "\n💡 Plugin operations are only allowed when CoCo is stopped.",
        )
        raise click.Abort()
...
def install(source: str, force: bool):
    ...
    Examples:
        coco plugin install examples/plugins/idealab-provider
        coco plugin install /path/to/plugin
        coco plugin install https://example.com/plugin.zip
    """
    from ..config.utils import get_plugins_dir

    # Check if CoCo is running
    _check_coco_not_running()
    ...
    click.echo("\n💡 Next steps:")
    click.echo("   1. Restart CoCo to load the plugin")
...
def uninstall(plugin_id: str):
    ...
    try:
        ...
        click.echo("💡 Restart CoCo to apply changes")
    except Exception as e:
