...
def _matches_coco_cli_command(command: str, *subcommands: str) -> bool:
    """Return whether command line looks like a CoCo CLI invocation."""
    ...
    return any(
        ...
        for pattern in (
            ...
            f'coco.exe" {subcommand}',
            f"coco.exe {subcommand}",
        )
    )


def _is_coco_service_command(command: str) -> bool:
    """Return whether the command line looks like a local CoCo app."""
    return _matches_coco_cli_command(command, "app")


def _is_coco_wrapper_process(name: str, command: str) -> bool:
    """Return whether the process looks like a CoCo CLI wrapper."""
    lowered_name = name.lower().removesuffix(".exe")
    return lowered_name == "coco" or _matches_coco_cli_command(
        command,
        "app",
        "desktop",
    )
...
def _candidate_hosts(host: str | None) -> list[str]:
    """Return host variants that can reach a local CoCo service."""
    if not host:
        return []
