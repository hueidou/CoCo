# -*- coding: utf-8 -*-
"""coco uninstall — remove the CoCo environment and CLI wrapper."""
...
def _remove_path_entry(profile: Path) -> bool:
    """
    Remove CoCo PATH lines from a shell profile. Returns True if changed.
    """
    if not profile.is_file():
        return False

    text = profile.read_text()
    # Remove the "# CoCo" comment line and the export PATH line
    cleaned = re.sub(
...
def uninstall_cmd(purge: bool, yes: bool) -> None:
    """Remove CoCo environment, CLI wrapper, and shell PATH entries."""
    wd = WORKING_DIR

    if purge:
        click.echo(f"This will remove ALL CoCo data in {wd}")
    else:
        click.echo(
            "This will remove the CoCo Python environment and CLI wrapper.",
        )
    ...
    click.echo("")
    click.echo("CoCo uninstalled. Please restart your terminal.")
