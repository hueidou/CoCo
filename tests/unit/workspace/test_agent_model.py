...
from coco.config.config import (
    AgentProfileConfig,
    AgentsRunningConfig,
    load_agent_config,
    save_agent_config,
)
from coco.constant import (
    LLM_BACKOFF_BASE,
    LLM_BACKOFF_CAP,
    LLM_MAX_RETRIES,
)
...
def mock_agent_workspace(tmp_path, monkeypatch):
    ...
    import json
    from coco.config.utils import get_config_path
    from coco.config.config import Config, AgentsConfig, AgentProfileRef

    # Setup workspace directory
    workspace_dir = tmp_path / "workspaces" / "test_agent"
    workspace_dir.mkdir(parents=True, exist_ok=True)

    # Patch WORKING_DIR so tests never touch the real ~/.coco/
    monkeypatch.setattr("coco.config.utils.WORKING_DIR", tmp_path)
    monkeypatch.setattr("coco.config.config.WORKING_DIR", tmp_path)
