...
from coco.__version__ import __version__
from coco.cli.main import cli
from coco.cli.update_cmd import (
    InstallInfo,
    RunningServiceInfo,
...
def _install_info(
    *,
    ...
    return InstallInfo(
        package_dir="/tmp/site-packages/coco",
...
@pytest.mark.parametrize(
    ...
    [
        ...
        (
            {
                "url": "file:///Users/test/CoCo",
                "dir_info": {"editable": True},
            },
            ("editable", "file:///Users/test/CoCo"),
        ),
        (
            {
                "url": "https://github.com/agentscope-ai/CoCo.git",
                "vcs_info": {"vcs": "git", "commit_id": "abc123"},
            },
            ("vcs", "https://github.com/agentscope-ai/CoCo.git"),
        ),
        (
            {"url": "file:///tmp/coco.whl"},
            ("local", "file:///tmp/coco.whl"),
        ),
        (
            {"url": "https://example.com/coco.whl"},
            ("direct-url", "https://example.com/coco.whl"),
        ),
    ],
)
...
def test_detect_installation(
    ...
    from coco.cli import update_cmd as update_cmd_module
...
def test_update_reports_up_to_date(monkeypatch) -> None:
    ...
    assert "CoCo is already up to date." in result.output
...
def test_probe_service_ignores_proxy_env(monkeypatch) -> None:
    ...
    monkeypatch.setattr("coco.cli.update_cmd.httpx.get", _fake_get)
...
def test_update_blocks_running_service(monkeypatch) -> None:
    ...
    assert result.exit_code != 0
    assert "Please stop it before running `coco update`" in result.output
    assert (
        "without `--yes` to confirm a forced `coco shutdown`" in result.output
    )
...
def test_update_can_cancel_forced_shutdown(monkeypatch) -> None:
    ...
    assert result.exit_code == 0
    assert (
        "forcibly terminate the current CoCo backend/frontend "
...
def test_update_can_force_shutdown_running_service(
    ...
    def _fake_shutdown(
        ...
        class _Result:
            returncode = 0
            stdout = "Stopped CoCo processes: 1234\n"
    ...
    assert result.exit_code == 0
    assert "Running `coco shutdown` before updating..." in result.output
    assert "Stopped CoCo processes: 1234" in result.output
    assert "Starting CoCo update..." in result.output
...
def test_update_worker_waits_for_launcher_exit(
    ...
    monkeypatch.setattr(
        "coco.cli.update_cmd._wait_for_process_exit",
...
def test_run_update_worker_detached_spawns_without_capture(
    ...
    monkeypatch.setattr(
        "coco.cli.update_cmd._spawn_update_worker",
        _fake_spawn,
    )
...
def test_update_worker_foreground_streams_output_and_cleans_plan(
    ...
    assert return_code == 0
    assert "[coco] Updating CoCo 1.0.0 -> 9.9.9..." in captured.out
    assert "[coco] Using installer: integration-test" in captured.out
    assert "installer: preparing" in captured.out
    assert "installer: done" in captured.out
    assert "[coco] Update completed successfully." in captured.out
...
def test_update_worker_foreground_propagates_failure_exit_code(
    ...
    assert "[coco] Update failed with exit code 7." in captured.out
    assert not plan_path.exists()
