...
REM CoCo Installer for Windows (cmd.exe / batch)
REM Usage: install.bat [-Version X.Y.Z] [-FromSource] [-SourceDir DIR]
REM                         [-Extras "llamacpp,mlx"] [-UvPath PATH] [-Help]
REM
REM Installs CoCo into %USERPROFILE%\.coco with a uv-managed Python environment.
...
REM ── Defaults ──────────────────────────────────────────────────────────────────
if defined COCO_HOME (
    set "COCO_HOME=%COCO_HOME%"
) else (
    set "COCO_HOME=%USERPROFILE%\.coco"
)
set "COCO_VENV=%COCO_HOME%\venv"
set "COCO_BIN=%COCO_HOME%\bin"
set "PYTHON_VERSION=3.12"
set "COCO_REPO=https://github.com/agentscope-ai/CoCo.git"
...
REM ──── Help ────────────────────────────────────────────────────────────────────────────────────────────────────────────────────────────────────────────
:show_help
echo CoCo Installer for Windows
echo.
...
echo.
echo Environment:
echo   COCO_HOME            Installation directory (default: %%USERPROFILE%%\.coco)
exit /b 0
...
echo [coco] Downloading uv ^(!_DL_ARCH!^) from GitHub Releases...
...
if not errorlevel 1 (
    ...
    echo [coco] curl failed, retrying with PowerShell...
    del "!_DL_ZIP!" >nul 2>&1
)
...
if errorlevel 1 (
    echo [coco] ERROR: GitHub download also failed.
    echo [coco] Download uv manually from: https://github.com/astral-sh/uv/releases/latest
    del "!_DL_ZIP!" >nul 2>&1
    exit /b 1
)
...
if defined ARG_UV_PATH (
    if not exist "%ARG_UV_PATH%" (
        echo [coco] ERROR: Specified uv not found: %ARG_UV_PATH%
        exit /b 1
    )
    for %%I in ("%ARG_UV_PATH%") do set "PATH=%%~dpI;!PATH!"
    echo [coco] uv found: %ARG_UV_PATH%
    goto :ensure_uv_done
)
...
for %%c in ("%USERPROFILE%\.local\bin\uv.exe" "%USERPROFILE%\.cargo\bin\uv.exe" "%LOCALAPPDATA%\uv\uv.exe") do (
    if exist %%c (
        ...
        echo [coco] uv found: %%~c
        goto :ensure_uv_done
    )
)

REM 3. Try astral.sh (standard installer, fast outside China)
echo [coco] Installing uv via astral.sh...
...
echo [coco] astral.sh unreachable, falling back to GitHub Releases...
call :download_uv_github
if errorlevel 1 (
    echo [coco] ERROR: Failed to install uv automatically.
    echo [coco] Please install uv manually: https://docs.astral.sh/uv/
    exit /b 1
)
goto :ensure_uv_done
...
where uv >nul 2>&1
if errorlevel 1 (
    echo [coco] ERROR: Failed to install uv. Please install it manually: https://docs.astral.sh/uv/
    exit /b 1
)
echo [coco] uv installed via astral.sh
...
set "_CONSOLE_DEST=%_REPO_DIR%\src\coco\console"
...
if exist "%_CONSOLE_SRC%\index.html" (
    echo [coco] Copying console frontend assets...
...
if not exist "%_REPO_DIR%\console\package.json" (
    echo [coco] WARNING: Console source not found - the web UI won't be available.
    exit /b 0
)

where npm >nul 2>&1
if errorlevel 1 (
    echo [coco] WARNING: npm not found - skipping console frontend build.
    echo [coco] WARNING: Install Node.js from https://nodejs.org/ then re-run this installer,
    echo [coco] WARNING: or run 'cd console ^&^& npm ci ^&^& npm run build' manually.
    exit /b 0
)

echo [coco] Building console frontend (npm ci ^&^& npm run build)...
pushd "%_REPO_DIR%\console"
npm ci
if errorlevel 1 (
    popd
    echo [coco] WARNING: npm ci failed - the web UI won't be available.
    exit /b 0
)
npm run build
if errorlevel 1 (
    popd
    echo [coco] WARNING: npm run build failed - the web UI won't be available.
    exit /b 0
)
popd
...
if exist "%_CONSOLE_SRC%\index.html" (
    ...
    set "CONSOLE_COPIED=1"
    set "CONSOLE_AVAILABLE=1"
    echo [coco] Console frontend built successfully
    exit /b 0
)

echo [coco] WARNING: Console build completed but index.html not found - the web UI won't be available.
exit /b 0
...
REM %~1 = RepoDir
if "%CONSOLE_COPIED%"=="1" (
    set "_CLEANUP_DEST=%~1\src\coco\console"
...
REM ══════════════════════════════ MAIN ═════════════════════════════════════════
:main
echo [coco] Installing CoCo into %COCO_HOME%
...
if exist "%COCO_VENV%" (
    echo [coco] Existing environment found, upgrading...
) else (
    echo [coco] Creating Python %PYTHON_VERSION% environment...
)

uv venv "%COCO_VENV%" --python %PYTHON_VERSION% --quiet --clear
if errorlevel 1 (
    echo [coco] ERROR: Failed to create virtual environment
    exit /b 1
)

set "VENV_PYTHON=%COCO_VENV%\Scripts\python.exe"
...
echo [coco] Python environment ready (%PY_VERSION%)

REM ──── Step 3: Install CoCo ──────────────────────────────────────────────────────────────────────────────────────────────────────────
set "EXTRAS_SUFFIX="
if defined ARG_EXTRAS set "EXTRAS_SUFFIX=[%ARG_EXTRAS%]"

set "VENV_COCO=%COCO_VENV%\Scripts\coco.exe"
...
goto :install_from_github_coco

:install_from_local
for %%I in ("%ARG_SOURCE_DIR%") do set "ARG_SOURCE_DIR=%%~fI"
echo [coco] Installing CoCo from local source: %ARG_SOURCE_DIR%
call :prepare_console "%ARG_SOURCE_DIR%"
echo [coco] Installing package from source...
...
call :cleanup_console "%ARG_SOURCE_DIR%"
if %_INST_ERR% neq 0 (
    echo [coco] ERROR: Installation from source failed
    exit /b 1
)
goto :install_verify

:install_from_github_coco
where git >nul 2>&1
if errorlevel 1 (
    echo [coco] ERROR: git is required for -FromSource without a local directory.
    echo [coco]        Please install Git from https://git-scm.com/ or pass a local path:
    echo [coco]        install-w-uv.bat -FromSource -SourceDir C:\path\to\CoCo
    exit /b 1
)
echo [coco] Installing CoCo from source (GitHub)...
set "CLONE_DIR=%TEMP%\coco-install-%RANDOM%"
git clone --depth 1 %COCO_REPO% "%CLONE_DIR%"
if errorlevel 1 (
    if exist "%CLONE_DIR%" rd /s /q "%CLONE_DIR%"
    echo [coco] ERROR: Failed to clone repository
    exit /b 1
)
...
if defined ARG_VERSION (
    ...
    set "_PACKAGE=coco%ARG_VERSION%"
)
rem === End Version Validation ===

echo [coco] Installing %_PACKAGE%%EXTRAS_SUFFIX% from PyPI...
...
uv pip install "%_PACKAGE%%EXTRAS_SUFFIX%" --python "%VENV_PYTHON%" --prerelease=allow --quiet --refresh-package coco
if errorlevel 1 (
    echo [coco] ERROR: Installation failed
    exit /b 1
)
...
if not exist "%VENV_COCO%" (
    echo [coco] ERROR: Installation failed: coco CLI not found in venv
    exit /b 1
)
echo [coco] CoCo installed successfully

REM Check console availability (for PyPI installs, probe the installed package)
if "%CONSOLE_AVAILABLE%"=="0" (
    "%VENV_PYTHON%" -c "import importlib.resources, coco; p=importlib.resources.files('coco')/'console'/'index.html'; print('yes' if p.is_file() else 'no')" > "%TEMP%\_coco_console_check.tmp" 2>&1
    set /p CONSOLE_CHECK=<"%TEMP%\_coco_console_check.tmp"
    del "%TEMP%\_coco_console_check.tmp" >nul 2>&1
    if "!CONSOLE_CHECK!"=="yes" set "CONSOLE_AVAILABLE=1"
)

REM ──── Step 4: Create wrapper scripts ────────────────────────────────────────────────────────────────────────────────────────
if not exist "%COCO_BIN%" mkdir "%COCO_BIN%"

REM PowerShell wrapper
set "WRAPPER_PS1=%COCO_BIN%\coco.ps1"
echo # CoCo CLI wrapper -- delegates to the uv-managed environment. > "%WRAPPER_PS1%"
echo $ErrorActionPreference = "Stop" >> "%WRAPPER_PS1%"
echo. >> "%WRAPPER_PS1%"
echo $CocoHome = if ($env:COCO_HOME) { $env:COCO_HOME } else { Join-Path $HOME ".coco" } >> "%WRAPPER_PS1%"
echo $RealBin = Join-Path $CocoHome "venv\Scripts\coco.exe" >> "%WRAPPER_PS1%"
echo. >> "%WRAPPER_PS1%"
echo if (-not (Test-Path $RealBin)) { >> "%WRAPPER_PS1%"
echo     Write-Error "CoCo environment not found at $CocoHome\venv" >> "%WRAPPER_PS1%"
echo     Write-Error "Please reinstall: irm ^<install-url^> ^| iex" >> "%WRAPPER_PS1%"
echo     exit 1 >> "%WRAPPER_PS1%"
echo } >> "%WRAPPER_PS1%"
echo. >> "%WRAPPER_PS1%"
echo ^& $RealBin @args >> "%WRAPPER_PS1%"
echo [coco] Wrapper created at %WRAPPER_PS1%

REM CMD wrapper
set "WRAPPER_CMD=%COCO_BIN%\coco.cmd"
echo @echo off > "%WRAPPER_CMD%"
echo REM CoCo CLI wrapper -- delegates to the uv-managed environment. >> "%WRAPPER_CMD%"
echo set "COCO_HOME=%%COCO_HOME%%" >> "%WRAPPER_CMD%"
echo if "%%COCO_HOME%%"=="" set "COCO_HOME=%%USERPROFILE%%\.coco" >> "%WRAPPER_CMD%"
echo set "REAL_BIN=%%COCO_HOME%%\venv\Scripts\coco.exe" >> "%WRAPPER_CMD%"
echo if not exist "%%REAL_BIN%%" ( >> "%WRAPPER_CMD%"
echo     echo Error: CoCo environment not found at %%COCO_HOME%%\venv ^>^&2 >> "%WRAPPER_CMD%"
echo     echo Please reinstall ^>^&2 >> "%WRAPPER_CMD%"
echo     exit /b 1 >> "%WRAPPER_CMD%"
echo ) >> "%WRAPPER_CMD%"
echo "%%REAL_BIN%%" %%* >> "%WRAPPER_CMD%"
echo [coco] CMD wrapper created at %WRAPPER_CMD%
...
set "check_str=;%COCO_BIN%;"
if /i "%path_check%" neq "%path_check:%check_str%=%" (
    echo [coco] %COCO_BIN% already in PATH
) else (
    :: === 修复1：安全传递参数（解决命令注入） ===
    if defined CURRENT_USER_PATH (
        powershell -NoProfile -Command "$p = $args[0]; $v = $args[1]; [Environment]::SetEnvironmentVariable('Path', $p + ';' + $v, 'User')" "%COCO_BIN%" "!CURRENT_USER_PATH!"
    ) else (
        powershell -NoProfile -Command "$p = $args[0]; [Environment]::SetEnvironmentVariable('Path', $p, 'User')" "%COCO_BIN%"
    )

    :: === 修复2：添加关键错误检查（解决失败不报错） ===
    if errorlevel 1 (
        echo [error] Failed to update PATH. COCO_BIN: "%COCO_BIN%"
        echo [error] Please verify the path is valid.
        exit /b 1
    )

    :: === 修复3：安全更新当前进程PATH ===
    set "PATH=%COCO_BIN%;!PATH!"
    echo [coco] Added %COCO_BIN% to PATH
)

REM ──── Done ────────────────────────────────────────────────────────────────────────────────────────────────────────────────────────────────────────────
echo.
echo CoCo installed successfully!
echo.
echo   Install location:  %COCO_HOME%
...
echo.
echo   coco init       # first-time setup
echo   coco app        # start CoCo
echo.
echo To upgrade later, re-run this installer.
echo To uninstall, run: coco uninstall
