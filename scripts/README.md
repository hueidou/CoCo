...
- Builds the console frontend (`console/`), copies `console/dist` to `src/coco/console/dist`, then builds the wheel. Output: `dist/*.whl`.
...
- Default tag: `coco:latest`. Uses `deploy/Dockerfile` (multi-stage: builds console then Python app).
- Example: `bash scripts/docker_build.sh myreg/coco:v1 --no-cache`.
