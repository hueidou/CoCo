# CoCo Installer for Windows (self-contained: includes uv download via GitHub)
# Usage: irm <url>/install.ps1 | iex
#    or: .\install.ps1 [-Version X.Y.Z] [-FromSource] [-SourceDir DIR]
#                            [-Extras "llamacpp,mlx"] [-UvPath PATH]
#
# Installs CoCo into ~/.coco with a uv-managed Python environment.
...
# ── Defaults ──────────────────────────────────────────────────────────────────
$CocoHome     = if ($env:COCO_HOME) { $env:COCO_HOME } else { Join-Path $HOME ".coco" }
$CocoVenv     = Join-Path $CocoHome "venv"
$CocoBin      = Join-Path $CocoHome "bin"
$PythonVersion = "3.12"
$CocoRepo     = "https://github.com/agentscope-ai/CoCo.git"

# ── Colors ────────────────────────────────────────────────────────────────────
function Write-Info { param([string]$Message) Write-Host "[coco] " -ForegroundColor Green  -NoNewline; Write-Host $Message }
function Write-Warn { param([string]$Message) Write-Host "[coco] " -ForegroundColor Yellow -NoNewline; Write-Host $Message }
function Write-Err  { param([string]$Message) Write-Host "[coco] " -ForegroundColor Red    -NoNewline; Write-Host $Message }
function Stop-WithError { param([string]$Message) Write-Err $Message; exit 1 }

# ── Help ──────────────────────────────────────────────────────────────────────
if ($Help) {
    @"
CoCo Installer for Windows
...
Environment:
  COCO_HOME            Installation directory (default: ~/.coco)
"@
    exit 0
}

Write-Host "[coco] " -ForegroundColor Green -NoNewline
Write-Host "Installing CoCo into " -NoNewline
Write-Host "$CocoHome" -ForegroundColor White
...
function Prepare-Console {
    ...
    $consoleDest = Join-Path $RepoDir "src\coco\console"
...
$VenvCoco = Join-Path $CocoVenv "Scripts\coco.exe"

if ($FromSource) {
    if ($SourceDir) {
        $SourceDir = (Resolve-Path $SourceDir).Path
        Write-Info "Installing CoCo from local source: $SourceDir"
        ...
        if (-not (Get-Command git -ErrorAction SilentlyContinue)) {
            Stop-WithError "git is required for -FromSource without a local directory. Please install Git from https://git-scm.com/ or pass a local path: .\install.ps1 -FromSource -SourceDir C:\path\to\CoCo"
        }
        Write-Info "Installing CoCo from source (GitHub)..."
        $cloneDir = Join-Path $env:TEMP "coco-install-$(Get-Random)"
        try {
            git clone --depth 1 $CocoRepo $cloneDir
    ...
    $package = "coco"
    if ($Version) { $package = "coco==$Version" }

    Write-Info "Installing ${package}${ExtrasSuffix} from PyPI..."
    uv pip install "${package}${ExtrasSuffix}" --python $VenvPython --prerelease=allow --quiet --refresh-package coco
    if ($LASTEXITCODE -ne 0) { Stop-WithError "Installation failed" }
}

# Verify the CLI entry point exists
if (-not (Test-Path $VenvCoco)) { Stop-WithError "Installation failed: coco CLI not found in venv" }

Write-Info "CoCo installed successfully"

# Check console availability (for PyPI installs, check the installed package)
if (-not $script:ConsoleAvailable) {
    $consoleCheck = & $VenvPython -c "import importlib.resources, coco; p=importlib.resources.files('coco')/'console'/'index.html'; print('yes' if p.is_file() else 'no')" 2>&1
    if ($consoleCheck -eq "yes") { $script:ConsoleAvailable = $true }
}

# ── Step 4: Create wrapper scripts ───────────────────────────────────────────
New-Item -ItemType Directory -Path $CocoBin -Force | Out-Null

$wrapperPath = Join-Path $CocoBin "coco.ps1"
$wrapperContent = @'
# CoCo CLI wrapper — delegates to the uv-managed environment.
$ErrorActionPreference = "Stop"

$CocoHome = if ($env:COCO_HOME) { $env:COCO_HOME } else { Join-Path $HOME ".coco" }
$RealBin   = Join-Path $CocoHome "venv\Scripts\coco.exe"

if (-not (Test-Path $RealBin)) {
    Write-Error "CoCo environment not found at $CocoHome\venv"
...
$cmdWrapperPath = Join-Path $CocoBin "coco.cmd"
$cmdWrapperContent = @"
@echo off
REM CoCo CLI wrapper — delegates to the uv-managed environment.
set "COCO_HOME=%COCO_HOME%"
if "%COCO_HOME%"=="" set "COCO_HOME=%USERPROFILE%\.coco"
set "REAL_BIN=%COCO_HOME%\venv\Scripts\coco.exe"
if not exist "%REAL_BIN%" (
    echo Error: CoCo environment not found at %COCO_HOME%\venv >&2
...
if (-not $isAlreadyAdded) {
    ...
    try {
        ...
        Write-Host ""
        Write-Host "ACTION REQUIRED: You must manually add the path to use CoCo."
...
# ── Done ──────────────────────────────────────────────────────────────────────
Write-Host ""
Write-Host "CoCo installed successfully!" -ForegroundColor Green
Write-Host ""

Write-Host "  Install location:  " -NoNewline; Write-Host "$CocoHome" -ForegroundColor White
...
Write-Host ""
Write-Host "  coco init" -ForegroundColor White -NoNewline; Write-Host "       # first-time setup"
Write-Host "  coco app"  -ForegroundColor White -NoNewline; Write-Host "        # start CoCo"
Write-Host ""
Write-Host "To upgrade later, re-run this installer."
Write-Host "To uninstall, run: " -NoNewline
Write-Host "coco uninstall" -ForegroundColor White
