...
info()  { printf "${GREEN}[coco]${RESET} %s\n" "$*"; }
warn()  { printf "${YELLOW}[coco]${RESET} %s\n" "$*"; }
error() { printf "${RED}[coco]${RESET} %s\n" "$*" >&2; }
die()   { error "$@"; exit 1; }

# ── Defaults ──────────────────────────────────────────────────────────────────
COCO_HOME="${COCO_HOME:-$HOME/.coco}"
COCO_VENV="$COCO_HOME/venv"
COCO_BIN="$COCO_HOME/bin"
PYTHON_VERSION="3.12"
COCO_REPO="https://github.com/agentscope-ai/CoCo.git"
...
printf "${GREEN}[coco]${RESET} Installing CoCo into ${BOLD}%s${RESET}\n" "$COCO_HOME"
...
uv venv "$COCO_VENV" --python "$PYTHON_VERSION" --quiet

# Verify the venv was created
[ -x "$COCO_VENV/bin/python" ] || die "Failed to create virtual environment"
info "Python environment ready ($("$COCO_VENV/bin/python" --version))"
...
## Ensure console frontend assets are in src/coco/console/ for source installs.
...
prepare_console() {
    ...
    local console_dest="$repo_dir/src/coco/console"
...
cleanup_console() {
    ...
    if [ "$_CONSOLE_COPIED" = 1 ]; then
        rm -rf "$repo_dir/src/coco/console/"*
    fi
}

if [ "$FROM_SOURCE" = true ]; then
    if [ -n "$SOURCE_DIR" ]; then
        info "Installing CoCo from local source: $SOURCE_DIR"
        prepare_console "$SOURCE_DIR"
        info "Installing package from source..."
        uv pip install "${SOURCE_DIR}${EXTRAS_SUFFIX}" --python "$COCO_VENV/bin/python" --prerelease=allow --index-url "$PYPI_MIRROR"
        cleanup_console "$SOURCE_DIR"
    else
        info "Installing CoCo from source (GitHub)..."
        CLONE_DIR="$(mktemp -d)"
        trap 'rm -rf "$CLONE_DIR"' EXIT
        git clone --depth 1 "$COCO_REPO" "$CLONE_DIR"
        prepare_console "$CLONE_DIR"
        info "Installing package from source..."
        uv pip install "${CLONE_DIR}${EXTRAS_SUFFIX}" --python "$COCO_VENV/bin/python" --prerelease=allow --index-url "$PYPI_MIRROR"
        # CLONE_DIR is cleaned up by trap; no need for cleanup_console
    fi
else
    PACKAGE="coco"
    if [ -n "$VERSION" ]; then
        PACKAGE="coco==$VERSION"
    fi

    info "Installing ${PACKAGE}${EXTRAS_SUFFIX} from PyPI..."
    uv pip install "${PACKAGE}${EXTRAS_SUFFIX}" --python "$COCO_VENV/bin/python" --prerelease=allow --quiet --index-url "$PYPI_MIRROR" --refresh-package coco
fi

# Verify the CLI entry point exists
[ -x "$COCO_VENV/bin/coco" ] || die "Installation failed: coco CLI not found in venv"
info "CoCo installed successfully"
...
if [ "$_CONSOLE_AVAILABLE" = 0 ]; then
    ...
    CONSOLE_CHECK="$("$COCO_VENV/bin/python" -c "import importlib.resources, coco; p=importlib.resources.files('coco')/'console'/'index.html'; print('yes' if p.is_file() else 'no')" 2>/dev/null || echo 'no')"
...
cat > "$COCO_BIN/coco" << 'WRAPPER'
#!/usr/bin/env bash
# CoCo CLI wrapper — delegates to the uv-managed environment.
set -euo pipefail

COCO_HOME="${COCO_HOME:-$HOME/.coco}"
REAL_BIN="$COCO_HOME/venv/bin/coco"
...
chmod +x "$COCO_BIN/coco"
info "Wrapper created at $COCO_BIN/coco"

# ── Step 5: Update PATH in shell profile ─────────────────────────────────────
PATH_ENTRY="export PATH=\"\$HOME/.coco/bin:\$PATH\""

add_to_profile() {
    local profile="$1"
    if [ -f "$profile" ] && grep -qF '.coco/bin' "$profile"; then
...
# ── Done ──────────────────────────────────────────────────────────────────────
echo ""
printf "${GREEN}${BOLD}CoCo installed successfully!${RESET}\n"
echo ""

# Install summary
printf "  Install location:  ${BOLD}%s${RESET}\n" "$COCO_HOME"
printf "  Python:            ${BOLD}%s${RESET}\n" "$("$COCO_VENV/bin/python" --version 2>&1)"
...
echo "Then run:"
echo ""
printf "  ${BOLD}coco init${RESET}       # first-time setup\n"
printf "  ${BOLD}coco app${RESET}        # start CoCo\n"
echo ""
printf "To upgrade later, re-run this installer.\n"
printf "To uninstall, run: ${BOLD}coco uninstall${RESET}\n"
