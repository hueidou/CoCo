...
### CoCo vs OpenClaw: Feature Comparison

Please check the [Comparison](/docs/comparison) page for detailed feature comparison.

### How to install CoCo

CoCo supports multiple installation methods. See
[Quick Start](https://coco.agentscope.io/docs/quickstart) for details:
...
To update CoCo, use the method matching your installation type:
...
5. If using the Windows Desktop App (exe), currently you need to uninstall and reinstall:
   - Uninstall CoCo from your PC
   - Download the latest version from: https://github.com/agentscope-ai/CoCo/releases
   - Reinstall

After upgrading, restart the service with `coco app`.

### How to initialize and start CoCo service
...
[Quick Start](https://coco.agentscope.io/docs/quickstart).

### Port 8088 conflict on Windows

On Windows, Hyper-V and WSL2 may reserve certain port ranges, which can conflict
with CoCo's default port **8088**. This affects all installation methods
...
- CoCo fails to start, or browser cannot connect to `http://127.0.0.1:8088/`
...
1. Run `coco app --port 8090` from a terminal instead
...
CoCo is open source. Official repository:
`https://github.com/agentscope-ai/CoCo`

### Where to check latest version upgrade details

See the site [Release notes](https://coco.agentscope.io/release-notes/?lang=en)
or CoCo GitHub [Releases](https://github.com/agentscope-ai/CoCo/releases).

### How to configure models

In Console, go to **Settings → Models** to configure. See the
[Models](https://coco.agentscope.io/docs/models) doc for details:
...
You can also use `coco models` for setup, downloads, and switching. See
[CLI → Models and environment variables → coco models](https://coco.agentscope.io/docs/cli#coco-models).

### How to use CoCo-Flash series models

CoCo-Flash is a family of models tuned by the CoCo team for CoCo's core
...
CoCo-Flash models are currently open-sourced on
...
All built-in local providers in CoCo can be used with CoCo-Flash models:

#### CoCo Local (llama.cpp)

In the CoCo Local model interface, simply choose a CoCo-Flash model to
...
> CoCo Local is still in beta. Compatibility across different devices and
> runtime stability are still being improved. If you run into issues while
> using it, please open an issue on GitHub.
> If CoCo Local does not work properly in your environment, we recommend
> deploying CoCo-Flash with Ollama or LM Studio first.

#### Ollama

1. Download a quantized CoCo-Flash model from
   ...
   [CoCo-Flash-4B-Q4_K_M](https://www.modelscope.cn/models/AgentScope/CoCo-Flash-4B-Q4_K_M).

   - Download with ModelScope CLI:

     ```bash
     modelscope download --model AgentScope/CoCo-Flash-4B-Q4_K_M README.md --local_dir ./dir
     ```
...
Create a text file named `coco-flash.txt` with the following contents. Replace
`/path/to/your/coco-xxx.gguf` with the absolute path to the `.gguf` file in
the CoCo-Flash model repository you downloaded:
...
1. In CoCo model settings, choose the Ollama provider, then automatically load
   the model on the Models page.

#### LM Studio

1. Follow step 1 in the Ollama section above to download an appropriate
   quantized CoCo-Flash model.
...
1. In CoCo model settings, choose the LM Studio provider, then automatically
   load the model on the Models page.

### When using models deployed with Ollama / LM Studio, why can't CoCo complete multi-turn interactions, complex tool calls, or remember earlier instructions?

In most cases, this is not a CoCo bug. The root cause is usually that the
model's context length is configured too small.

When you deploy a local model with Ollama or LM Studio, if the model's
`context length` is too low, CoCo may show problems such as:
...
- Before running CoCo, set the model's `context length` to **at least 32K**
- For more complex tasks, frequent tool calls, or longer conversations, you
  may need a value **higher than 32K**

> ⚠️ **Before running CoCo, you must set the context length to 32K or higher**
>
> For local models deployed with Ollama or LM Studio, CoCo typically needs a
...
The easiest way to create a cron job is to talk to CoCo in the channel where you want the results. For example, say: “Create a scheduled task that reminds me to drink water every five minutes.” You can then see the enabled job in Console.

If a scheduled task does not run as expected, try the following:

1. Confirm that the CoCo service is running.
...
6. To verify that the task was created and can run, click **Execute Now**. If it works, you should see the reply in the target channel. You can also ask CoCo: “Trigger the ‘drink water reminder’ task I just created.”
...
[Skills](https://coco.agentscope.io/docs/skills).

### How to configure MCP

Go to **Agent -> MCP** in Console. You can enable/disable/delete/create MCP
clients there. See [MCP](https://coco.agentscope.io/docs/mcp).
...
CoCo supports API keys obtained via DashScope Coding Plan. If it still fails,
please check:
...
[issue](https://github.com/agentscope-ai/CoCo/issues) in the CoCo GitHub
