...
CoCo supports **multi-agent**, allowing you to run multiple independent AI agents in a single CoCo instance.
...
Simply put, **multi-agent** lets you run multiple "personas" in one CoCo, where each persona:
...
You might use CoCo across multiple platforms:
...
After starting CoCo, you'll see the **Agent Selector** in the **top-left corner** of the console:
...
**Note**: After deletion, the workspace directory is retained (to prevent accidental data loss). To completely remove it, manually delete the `~/.coco/workspaces/{agent_id}` directory.
...
If you previously used CoCo **v0.0.x**, upgrading to **v0.1.0** will **automatically migrate**:
...
2. **Verify Migration**
   ...
   - After starting CoCo, check the agent list in console
...
coco skills config --agent-id abc123
...
```
~/.coco/workspaces/{agent_id}/PROFILE.md
```
...
# Workspace: ~/.coco/workspaces/code_reviewer
...
Workspace: ~/.coco/workspaces/writer_bot
Profile: [Auto-generated detailed capability description]
```
...
coco channels list --agent-id abc123
coco cron list --agent-id abc123
coco skills list --agent-id abc123
...
- `coco channels` - channel management
- `coco cron` - cron jobs
- `coco daemon` - runtime status
- `coco chats` - chat management
- `coco skills` - skill management

**Commands NOT Supporting `--agent-id`** (global operations):

- `coco init` - initialization
- `coco providers` - model providers
- `coco models` - model configuration
- `coco env` - environment variables
...
cp -r ~/.coco/workspaces/abc123 ~/backups/agent-abc123-$(date +%Y%m%d)

# Backup all agents
cp -r ~/.coco/workspaces ~/backups/workspaces-$(date +%Y%m%d)
```
