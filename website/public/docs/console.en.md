...
The **Console** is CoCo's built-in web interface. After running `coco app`,
open `http://127.0.0.1:8088/` in your browser to enter the Console.

**In the Console, you can:**

- Chat with CoCo in real time
- Enable/disable/configure messaging channels
- View and manage all chat sessions
- Manage scheduled jobs and heartbeat
- Edit CoCo's persona and behavior files
- Enable/import skills to extend CoCo's capabilities
...
This is where you talk to CoCo. It is the default page when the Console opens.
...
button (↑). CoCo replies in real time.
...
Create and manage scheduled jobs that CoCo runs automatically by time.

![Cron Jobs](https://img.alicdn.com/imgextra/i2/O1CN018jn4Wm1C9SBJy58mo_!!6000000000038-2-tps-3822-2064.png)

**Create a new job:**

> If the cron job fails to be created, please refer to the **Troubleshooting Scheduled (Cron) Tasks** section in the [FAQ](https://coco.agentscope.io/docs/faq) to identify the cause.

The **simplest way to create a cron job is to chat directly with CoCo** and let it handle the creation for you. For example, if you want to receive a reminder to drink water on DingTalk, simply message CoCo on DingTalk: "Help me create a cron job to remind me to drink water every 5 minutes." Once created, you can view the new task on the Cron Jobs page in the console.
...
2. Fill in each section:
   ...
   - **Task type & content** — **Text**: send fixed text from **Message content**.
     **Agent**: fill **Request content**; on each run CoCo receives the text
...
tick, send the contents of `HEARTBEAT.md` as a user message to CoCo, and
...
Edit files that define CoCo's persona and behavior — `SOUL.md`, `AGENTS.md`,
`HEARTBEAT.md`, etc. — directly in the browser.

> **Multi-agent:** Starting from **v0.1.0**, CoCo supports **multi-agent** mode.
> You can run multiple independent agents in one CoCo instance, each with its own
...
Manage skills that extend CoCo (e.g. read PDF, create Word, fetch news). More
...
Manage runtime environment variables needed by CoCo tools and skills (e.g.
`TAVILY_API_KEY`).
...
> **Note:** Variable validity is your responsibility. CoCo only stores and loads
> values.
>
...
> Data is stored in `~/.coco/token_usage.json`. Override the filename with
> `COCO_TOKEN_USAGE_FILE`. See [Config — Environment variables](./config#environment-variables).
