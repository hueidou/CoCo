...
CoCo includes built-in security features to protect your agent from malicious inputs and unsafe skills. These are configured in the Console under **Settings → Security**, or via `config.json`.

## Overview

CoCo's security system consists of three core security layers:
...
| Field            | Description                                                                                                                                           |
| ---------------- | ----------------------------------------------------------------------------------------------------------------------------------------------------- |
| `enabled`        | Enable or disable Tool Guard entirely. Can also be set via the `COCO_TOOL_GUARD_ENABLED` environment variable (takes precedence).                    |
...
**Default protection**: The `{WORKING_DIR}.secret/` directory (which stores API keys, authentication credentials, and provider configurations) is included in the sensitive-file list by default. By default, `WORKING_DIR` is `~/.coco/`, making the full path `~/.coco.secret/`.
...
```json
{
  "security": {
    "file_guard": {
      "enabled": true,
      "sensitive_files": ["~/.ssh/", "/etc/passwd", "~/.coco.secret/"]
    }
  }
}
```
...
**Configuration priority**: Environment variable `COCO_SKILL_SCAN_MODE` > Console settings > `config.json`
...
The scanner uses YAML rule files in `src/coco/security/skill_scanner/rules/signatures/`. You can customize the scan policy via a YAML policy file:

```python
from coco.security.skill_scanner import SkillScanner
from coco.security.skill_scanner.scan_policy import ScanPolicy
...
CoCo supports optional web login authentication to protect the Console from unauthorized access. Authentication is **disabled by default** and must be explicitly enabled via the `COCO_AUTH_ENABLED` environment variable.

![login](https://img.alicdn.com/imgextra/i3/O1CN01SP7Ppd289g5e9kKO6_!!6000000007890-2-tps-3822-2064.png)

### How it works

1. **Enable authentication** — Set `COCO_AUTH_ENABLED=true` and start CoCo
2. **Registration flow**:
...
4. **Auto-registration** (optional):
   - Set `COCO_AUTH_USERNAME` and `COCO_AUTH_PASSWORD` environment variables
   - CoCo automatically creates the admin account on startup, skipping web registration
   - Useful for Docker, Kubernetes, server management panels, and other automated deployments
5. **Localhost bypass** — Requests from localhost (`127.0.0.1` / `::1`) automatically skip authentication; CLI commands (`coco app`, `coco chat`, etc.) work without a token
...
| --------------------- | -------------------------------------------- | -------- |
| `COCO_AUTH_ENABLED`  | Set to `true` to enable authentication       | **Yes**  |
| `COCO_AUTH_USERNAME` | Pre-set admin username for auto-registration | Optional |
| `COCO_AUTH_PASSWORD` | Pre-set admin password for auto-registration | Optional |

**Configuration notes**:

- `COCO_AUTH_ENABLED=true` is the only required variable to enable authentication
- `COCO_AUTH_USERNAME` and `COCO_AUTH_PASSWORD` are used together:
...
export COCO_AUTH_ENABLED=true
coco app

# Or: Auto-registration mode
export COCO_AUTH_ENABLED=true
export COCO_AUTH_USERNAME=admin
export COCO_AUTH_PASSWORD=mypassword
coco app
```
...
```cmd
set COCO_AUTH_ENABLED=true
rem Optional: auto-registration
rem set COCO_AUTH_USERNAME=admin
rem set COCO_AUTH_PASSWORD=mypassword
coco app
```
...
```powershell
$env:COCO_AUTH_ENABLED = "true"
# Optional: auto-registration
# $env:COCO_AUTH_USERNAME = "admin"
# $env:COCO_AUTH_PASSWORD = "mypassword"
coco app
```
...
```bash
docker run -e COCO_AUTH_ENABLED=true \
  -e COCO_AUTH_USERNAME=admin \
  -e COCO_AUTH_PASSWORD=mypassword \
...
> **Tip**: To skip auto-registration, remove `COCO_AUTH_USERNAME` and `COCO_AUTH_PASSWORD` and register via browser on first visit.
...
services:
  coco:
    image: agentscope/coco:latest
    ports:
      - "127.0.0.1:8088:8088"
    environment:
      - COCO_AUTH_ENABLED=true
      - COCO_AUTH_USERNAME=admin
      - COCO_AUTH_PASSWORD=mypassword
    volumes:
      - coco-data:/app/working
      - coco-secrets:/app/working.secret
```
...
Then pass it to Docker with `--env-file .env`, or source it in your shell before running `coco app`.

### Disable authentication

Remove or unset the environment variable and restart CoCo:

```bash
# Linux / macOS
unset COCO_AUTH_ENABLED
coco app

# Docker — simply remove the -e flag. The example below includes volumes for persistence.
docker run -p 127.0.0.1:8088:8088 -v coco-data:/app/working -v coco-secrets:/app/working.secret agentscope/coco:latest
```
...
```bash
docker exec -it <container_name> coco auth reset-password
```
...
rm ~/.coco.secret/auth.json  # or $WORKING_DIR.secret/auth.json
# Restart CoCo; re-register on next visit
coco app
```
