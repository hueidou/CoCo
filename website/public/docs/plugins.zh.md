...
1. **只安装可信插件**：插件代码会在 CoCo 进程中执行
2. **检查依赖**：确保插件依赖来自可信源
3. **审查代码**：安装前审查插件源代码
4. **离线操作**：插件安装/卸载需要 CoCo 离线
...
对于需要修改 CoCo 行为的插件（如自定义命令），可以使用 monkey patch：
