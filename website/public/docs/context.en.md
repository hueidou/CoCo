...
CoCo context management uses two parallel offload paths to handle the limited context window:
...
CoCo's context consists of two components:
...
- [MemoryCompactionHook](https://github.com/agentscope-ai/CoCo/blob/main/src/coco/agents/hooks/memory_compaction.py)
...
When the context approaches its limit, CoCo automatically triggers compaction, condensing old conversations into a structured summary.
...
Configuration is located in `~/.coco/config.json` under `agents.running`:
