# CoCo Desktop Application Guide
...
This guide explains how to install and use the CoCo Desktop application on Windows and macOS.

[releases]: https://github.com/agentscope-ai/CoCo/releases
...
1. **Download the installer**
   Download `CoCo-Setup-<version>.exe` from the [Release page][releases]

2. **Run the installer**
   Double-click the `.exe` file and follow the installation wizard
   - Default installation location: `C:\Users\<your-username>\AppData\Local\CoCo`
...
#### **CoCo Desktop** (Recommended for daily use)
...
- **How to Launch**: Double-click the "CoCo Desktop" icon on desktop or Start Menu
- **Technical Note**: Uses VBScript launcher, runs Python process in background

#### **CoCo Desktop (Debug)** (Debug Mode)
...
- **How to Launch**: Double-click "CoCo Desktop (Debug)" icon in Start Menu
- **Log Contents**:
...
A: Use "CoCo Desktop (Debug)" mode to view terminal output for error messages

**Q: How to uninstall?**
A: Go to Windows Settings → Apps → Installed apps → Find "CoCo Desktop" → Uninstall
...
1. **Download the archive**
   Download `CoCo-<version>-macOS.zip` from the [Release page][releases]

2. **Extract**
   Double-click the `.zip` file to extract and get `CoCo.app`

3. **Move to Applications folder (Optional)**
   Drag `CoCo.app` to the `/Applications` folder

### First Launch: Bypassing System Security Restrictions

#### Why manual trust is needed?

CoCo is **not Apple Developer-signed or notarized**, so macOS Gatekeeper will block it by default.
...
1. **Right-click** (or Control + click) on `CoCo.app`
...
2. Scroll down to find a message like:
   _"'CoCo' was blocked from use because it cannot verify the developer"_
...
- Double-click `CoCo.app` to launch
- The app runs in the background and opens a browser window
- Logs are written to: `~/.coco/desktop.log`
...
cd /Applications  # or wherever your CoCo.app is located

# Set environment variables and launch
APP_ENV="$(pwd)/CoCo.app/Contents/Resources/env"
PYTHONPATH= PYTHONHOME="$APP_ENV" "$APP_ENV/bin/python" -m coco desktop
```
...
1. Check the `~/.coco/desktop.log` file for errors
...
**Q: How to uninstall?**
A: Drag `CoCo.app` to the Trash, then delete the `~/.coco` configuration folder
...
- **GitHub Issues**: [Submit an issue](https://github.com/agentscope-ai/CoCo/issues)
- **Packaging documentation**: `scripts/pack/README.md` - Technical details and local build guide
- **Log locations**:
  - Windows: View in Debug mode terminal, or `%USERPROFILE%\.coco\` directory
  - macOS: `~/.coco/desktop.log`
