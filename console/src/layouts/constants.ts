...
export const PYPI_URL = "https://pypi.org/pypi/coco/json";

export const GITHUB_URL = "https://github.com/agentscope-ai/CoCo" as const;
...
export const getDocsUrl = (lang: string): string =>
  `https://coco.agentscope.io/docs/intro?lang=${getWebsiteLang(lang)}`;

export const getFaqUrl = (lang: string): string =>
  `https://coco.agentscope.io/docs/faq?lang=${getWebsiteLang(lang)}`;

export const getReleaseNotesUrl = (lang: string): string =>
  `https://coco.agentscope.io/release-notes?lang=${getWebsiteLang(lang)}`;
...
docker run -p 127.0.0.1:8088:8088 -v coco-data:/app/working agentscope/coco:latest
\`\`\`

升级后重启服务 coco app。`,

  ru: `### Как обновить CoCo

Чтобы обновить CoCo, выберите способ в зависимости от типа установки:
...
После обновления перезапустите сервис с помощью \`coco app\`.`,

  en: `### How to update CoCo
